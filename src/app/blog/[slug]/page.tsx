...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <article><header className="article-hero"><div className="container article-hero-inner"><Breadcrumbs items={crumbs} /><span className="eyebrow">{categoryLabels[post.category] || post.category}</span><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><div className="article-meta"><span>Sindikat Studio 83</span>{post.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></div></header><div className="article-cover"><Image src={post.cover} width={1280} height={720} priority alt={post.coverAlt} /></div><section className="section"><div className="container article-layout"><aside className="article-aside"><strong>U tekstu</strong><nav><a href="#article-content">Čitanje</a><a href="#povezani">Povezani tekstovi</a><Link href="/kontakt/">Primijeni na projekat</Link></nav></aside><div className="article-content" id="article-content" dangerouslySetInnerHTML={{ __html: cleanArticleBody(post.body, post.cover) }} /></div></section></article>
